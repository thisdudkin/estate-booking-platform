...
public record FavoriteListingRemovedEvent(
    UUID eventId,
    UUID aggregateId,
    UUID tenantId,
    UUID listingId,
    ...
    private static final String EVENT_TYPE = "FAVORITE_LISTING_REMOVED";

    public static FavoriteListingRemovedEvent of(
        UserId tenantId,
        ListingId listingId,
        Instant occurredAt
    ) {
        return new FavoriteListingRemovedEvent(
            UUID.randomUUID(),
            tenantId.value(),
            tenantId.value(),
            listingId.value(),
